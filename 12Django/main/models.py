...
class Course(models.Model):
    ...
    students = models.ManyToManyField(Students, blank=True, verbose_name='Студенты', related_name='courses_taken')
...
class Mark(models.Model):
    mark = models.SmallIntegerField(validators=[MinValueValidator(0), MaxValueValidator(100)], verbose_name='Оценка')
    student = models.ForeignKey(Students, on_delete=models.CASCADE)
    course = models.ForeignKey(Course, on_delete=models.CASCADE)

    def __str__(self):
        return f"{self.course} - {self.mark} - {self.student}"
